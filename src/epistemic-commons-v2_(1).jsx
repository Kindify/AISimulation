...
// ─── GAME LOGIC ──────────────────────────────────────────
function computeOutcome(crisis, decisions) {
  let scores = { integrity: 0, trust: 0, legitimacy: 0, rights: 0 };
  const chosenOptions = {};
...
function computeCounterfactuals(crisis, decisions, currentOutcome) {
  ...
  counterfactuals.sort((a, b) => Math.abs(b.diff) - Math.abs(a.diff));
...
// ─── UI HELPERS ──────────────────────────────────────────
function MetricBar({ label, value, maxVal = 30, icon, color }) {
...
function RoleCard({ role, isActive, isCompleted }) {
  return (
...
// ─── MAIN ────────────────────────────────────────────────
export default function EpistemicCommonsV2() {
  const [phase, setPhase] = useState("intro");
  const [crises, setCrises] = useState(BUILT_IN_CRISES);
  ...
  const addGeneratedScenario = () => {
    ...
    setCrises(prev => [...prev, genPreview]);
...
textarea, input { font-family: 'DM Sans', sans-serif; }`;
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                {crises.map((c, i) => (
                  <div key={c.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", background: "#0c0f14", borderRadius: 6, border: "1px solid #1e2533" }}>
                    ...
                    <span style={{ fontFamily: "'DM Mono', monospace", fontSize: 9, color: i < BUILT_IN_CRISES.length ? "#64748b" : "#a855f7" }}>{i < BUILT_IN_CRISES.length ? "BUILT-IN" : "AI-GENERATED"}</span>
                  </div>
                ))}
              </div>
            </div>
  ...
  if (phase === "roleplay") {
    ...
    return (
      <div style={S}><style>{css}</style>{hdr}
        <div style={{ display: "grid", gridTemplateColumns: "200px 1fr", gap: 20, maxWidth: 960, margin: "0 auto" }}>
  ...
  if (phase === "resolution") {
    ...
    const gradeColor = { A: "#10b981", B: "#06b6d4", C: "#f59e0b", D: "#f97316", F: "#ef4444" }[outcome.coordinationGrade] || "#64748b";
        ...
        <div style={{ maxWidth: 760, margin: "0 auto" }}>
          ...
          {outcome.triggeredInteractions.length > 0 && (
            <div style={{ marginBottom: 16 }}>
              ...
              {outcome.triggeredInteractions.map((inter, i) => (
              ...
              <div style={{ marginTop: 10 }}>
                {ROLES.map(r => (
                  <div key={r.id} style={{ background: "#141820", border: `1px solid ${r.color}22`, borderRadius: 10, padding: 14, marginBottom: 8 }}>
                    ...
                    {crisis.roleIntel[r.id].bullets.map((b, i) => (
  ...
  // ─── DEBRIEF ───────────────────────────────
  if (phase === "debrief") {
    const totalScores = { integrity: 0, trust: 0, legitimacy: 0, rights: 0 };
    ...
    const avgGradeVal = grades.reduce((s, g) => s + ({ A: 4, B: 3, C: 2, D: 1, F: 0 }[g] || 0), 0) / grades.length;
    const overallGrade = avgGradeVal >= 3.5 ? "A" : avgGradeVal >= 2.5 ? "B" : avgGradeVal >= 1.5 ? "C" : avgGradeVal >= 0.5 ? "D" : "F";
    const gradeColor = { A: "#10b981", B: "#06b6d4", C: "#f59e0b", D: "#f97316", F: "#ef4444" }[overallGrade] || "#64748b";
    ...
    const totalSynergies = outcomes.reduce((s, o) => s + o.triggeredInteractions.filter(i => i.type === "synergy").length, 0);
    const totalConflicts = outcomes.reduce((s, o) => s + o.triggeredInteractions.filter(i => i.type === "conflict").length, 0);
      ...
      <div style={S}><style>{css}</style>{hdr}
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          ...
          {crises.map((c, ci) => {
            ...
            const gc = { A: "#10b981", B: "#06b6d4", C: "#f59e0b", D: "#f97316", F: "#ef4444" }[o.coordinationGrade] || "#64748b";
            return (
              <div key={c.id} style={{ background: "#141820", border: "1px solid #1e2533", borderRadius: 10, padding: 16, marginBottom: 10 }}>
                ...
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                  {o.triggeredInteractions.map((inter, i) => (
          ...
          <button onClick={restart} style={{ width: "100%", padding: "14px 24px", background: "#141820", color: "#e2e8f0", border: "1px solid #1e2533", borderRadius: 8, cursor: "pointer", fontFamily: "'Space Mono', monospace", fontSize: 13, fontWeight: 700, letterSpacing: 2, textTransform: "uppercase" }}>
